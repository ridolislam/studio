...
export default function LeadPulseDashboard() {
  ...
  const handleStart = async () => {
    ...
    try {
      // Send entire batch to the distributed Render endpoint
      const res = await validateBatchDistributed({ numbers: lines, email });
      
      if (res.success && res.results) {
        const batchResults: ValidationResult[] = res.results.map((item: any) => ({
        ...
        // Update counts
        const newCounts = { mobile: 0, landline: 0, invalid: 0 };
        batchResults.forEach(r => {
          if (r.status === 'invalid') newCounts.invalid++;
          else if (r.type.toLowerCase().includes('mobile')) newCounts.mobile++;
          else newCounts.landline++;
        });
        setCounts(newCounts);
        
        // Show last result as live JSON
        setLiveJson(res.results[res.results.length - 1]);
        setProgress(100);
        toast({ title: 'Success', description: `Processed ${batchResults.length} numbers.` });
      } else {
        toast({ variant: 'destructive', title: 'Batch Error', description: res.message || "Failed to process batch." });
      }
    } catch (err) {
      toast({ variant: 'destructive', title: 'Network Error', description: "Failed to connect to backend." });
    } finally {
            ...
            <div className="xl:col-span-1 space-y-6">
              <Card className="border-white/10 bg-card shadow-2xl relative overflow-hidden">
                ...
                <CardContent className="space-y-4">
                  ...
                  />
                  <Button onClick={handleStart} disabled={isProcessing} className="w-full h-14 bg-primary font-black italic rounded-xl shadow-lg shadow-primary/20 transition-all">
                    {isProcessing ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Play className="h-4 w-4 mr-2" />} 
                    {isProcessing ? "DISTRIBUTING..." : "START DISTRIBUTED"}
                  </Button>
                </CardContent>
              </Card>
              ...
              <Card className="border-white/5 bg-black/40 rounded-2xl overflow-hidden shadow-xl">
                <div className="bg-white/5 p-4 border-b border-white/5 flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest">Worker Response</span>
                </div>
                <ScrollArea className="h-[280px] p-4 font-code text-[10px] text-green-400 bg-black/60">
                  {liveJson ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full opacity-20 space-y-2">
                      ...
                      <span className="italic">Waiting for batch...</span>
                    </div>
                  )}
                </ScrollArea>
              </Card>
            ...
            <div className="xl:col-span-3 space-y-6">
              ...
              <Card className="bg-card/60 backdrop-blur-xl border-white/5 rounded-3xl overflow-hidden shadow-2xl">
                <div className="p-4 border-b border-white/5 bg-white/5 flex items-center justify-between">
                  <span className="text-xs font-black uppercase tracking-widest opacity-70">Distributed Batch Results</span>
                  <Button 
                    size="sm" 
                    variant="outline" 
                      ...
                      {results.length === 0 ? (
                        <TableRow>
                          <TableCell colSpan={5} className="h-64 text-center opacity-20">
                            ...
                            <p className="font-black italic uppercase">Results will appear here in batch</p>
                          </TableCell>
                        </TableRow>
                      ) : (
                        results.map(res => (
                          <TableRow key={res.id} className="border-white/5 h-16 hover:bg-white/5 transition-all">
                            ...
                            <TableCell className="text-right px-8">
                              <Zap className={cn("h-4 w-4 ml-auto", res.status === 'success' ? "text-primary" : "text-muted-foreground")} />
                            </TableCell>
                          </TableRow>
                        ))
                      )}
